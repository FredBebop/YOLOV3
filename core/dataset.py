...
class Dataset(object):
    ...
    def parse_annotation(self, annotation):
        ...
        bboxes = np.array([box for box in line[4:]]).reshape((-1,5))
...
class Data_preprocess(object):
    ...
    def load_labels(self):
        ...
        txtname=os.path.join(self.data_path,'xmlpath.txt')
        with open(txtname, 'r') as f:
            image_ind = [x.strip('.xml\n').split('\\')[-1] for x in f.readlines()]
        ...
        for ind in image_ind:
            ...
            if len(class_inds) == 0:
                ...
                with open(os.path.abspath('..')+"\data\dataset\\train.txt", "a") as f:
                    f.write(str(my_index)+" "+os.path.abspath('..') + '\data\A05\JPEGImages\\'+ind+'.jpg'+" "+str(img_width) + " " + str(img_height) + annotation_label + "\n")
    ...
    def load_data(self, index):
        ...
        filename = os.path.join(os.path.abspath('..'), 'data\A05\Annotations', index + '.xml')
...
if __name__ == "__main__":
    data_path = os.path.join(os.path.abspath('..'), "data\dataset")
